      ...
      <div className="bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 py-24 text-white">
        <div className="mx-auto max-w-7xl px-4">
          ...
          <h1 className="max-w-4xl text-4xl font-black md:text-6xl">
            Deploy, manage and scale your applications securely.
          </h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300">
            We handle hosting, deployment, CI/CD, server setup, monitoring,
            backups and performance optimization for production apps.
          </p>
          ...
          >
            Setup Cloud <ArrowRight size={18} />
          </a>
        </div>
      </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-20 md:grid-cols-3">
        {features.map((item) => (
          <div key={item} className="rounded-[28px] border bg-white p-7 shadow-sm">
            <Cloud className="mb-5 text-indigo-600" size={34} />
            <h3 className="text-xl font-black text-slate-950">{item}</h3>
            <p className="mt-4 text-slate-500">
              Reliable infrastructure setup for fast and secure applications.
            </p>
          </div>
        ))}
      </div>
